...
def get_api_key(key_name, required=False):
    """Get API key from st.secrets first, then fall back to environment variables"""
    try:
        return st.secrets[key_name]
    except (KeyError, FileNotFoundError):
        value = os.getenv(key_name)
        if required and not value:
            st.error(f"❌ {key_name} is required but not found in secrets or environment variables")
            st.stop()
        return value
...
# Try to get GROQ API key from secrets first, then fallback to manual input
groq_api_key = get_api_key("GROQ_API_KEY")
if groq_api_key:
    st.sidebar.success("✅ Using GROQ API Key from secrets")
    api_key = groq_api_key
else:
    api_key = st.sidebar.text_input("Enter your Groq API Key:", type="password", 
                                   help="Add GROQ_API_KEY to secrets for automatic configuration")
...
if GOOGLE_API_KEY and GOOGLE_CSE_ID:
    ...
    st.sidebar.warning("🔍 Google Search: Using fallback")
st.sidebar.info("📚 Arxiv & Wikipedia: Always available")
